...
def validate_interactive_message(request: OutboundMessageRequest) -> None:
    ...
    if not request.interactive_type:
        raise ValidationError("interactive_type is required for INTERACTIVE message_type")

    try:
        int_type = InteractiveType(request.interactive_type)
    except ValueError:
        raise ValidationError(f"Invalid interactive_type: {request.interactive_type}") from None

    if not request.text:
        raise ValidationError("text (body) is required for interactive messages")
...
def _validate_button(request: OutboundMessageRequest) -> None:
    ...
    if not request.buttons:
        raise ValidationError("buttons is required for BUTTON interactive type")

    if len(request.buttons) > MAX_BUTTONS_PER_MESSAGE:
        raise ValidationError(f"Maximum {MAX_BUTTONS_PER_MESSAGE} buttons allowed")

    for i, btn in enumerate(request.buttons):
        if not isinstance(btn, dict) or "id" not in btn or "title" not in btn:
            raise ValidationError(f"Button {i} must have 'id' and 'title' fields")
        if len(btn["title"]) > MAX_BUTTON_TEXT_LENGTH:
            raise ValidationError(f"Button {i} title exceeds {MAX_BUTTON_TEXT_LENGTH} chars")
...
def _validate_flow(request: OutboundMessageRequest) -> None:
    ...
    if missing:
        raise ValidationError(f"{', '.join(missing)} required for FLOW interactive type")

    if request.flow_cta and len(request.flow_cta) > MAX_BUTTON_TEXT_LENGTH:
        raise ValidationError(f"flow_cta exceeds {MAX_BUTTON_TEXT_LENGTH} characters")


def _validate_cta_url(request: OutboundMessageRequest) -> None:
    """Valida tipo CTA_URL."""
    if not request.cta_url:
        raise ValidationError("cta_url is required for CTA_URL interactive type")

    if not request.cta_display_text:
        raise ValidationError("cta_display_text is required for CTA_URL interactive type")

    if len(request.cta_display_text) > MAX_BUTTON_TEXT_LENGTH:
        raise ValidationError(f"cta_display_text exceeds {MAX_BUTTON_TEXT_LENGTH} characters")

    if request.buttons:
        raise ValidationError("buttons not allowed for CTA_URL interactive type")


def _validate_location_request(request: OutboundMessageRequest) -> None:
    """Valida tipo LOCATION_REQUEST_MESSAGE."""
    if request.buttons:
        raise ValidationError("buttons not allowed for LOCATION_REQUEST_MESSAGE type")
