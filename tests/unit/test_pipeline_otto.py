...
def make_message(msg_text: str, ts: int, message_id: str = "m1", chat_id: str | None = None) -> NormalizedWhatsAppMessage:
    ...
    if chat_id is not None:
        # chat_id is not a formal field of NormalizedWhatsAppMessage but pipeline reads it via getattr
...
def test_otto_prefix_on_first_message_of_day() -> None:
    ...
    config = PipelineConfig(dedupe_store=dedupe, session_store=session_store, orchestrator=orchestrator)
    pipeline = WhatsAppInboundPipeline(config)

    session = pipeline._get_or_create_session(make_message("oi", str(int(datetime.now(tz=timezone.utc).timestamp()))), sender_phone=None)
